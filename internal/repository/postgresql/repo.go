...
func (r *GoodsRepository) CreatePackage(ctx context.Context, pkg *models.Package) error {
	...
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to create package: %v", err))
		return err
	}
	return nil
}
...
func (r *GoodsRepository) AddProductToPackage(ctx context.Context, packageID int64, products []models.PackageContent) error {
	...
	_, err := br.Exec()
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to add products to package: %v", err))
		return err
	}
	return nil
}
...
func (r *GoodsRepository) DeletePackage(ctx context.Context, packageID int64) error {
	...
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to begin transaction: %v", err))
		return err
	}
	defer tx.Rollback(ctx)

	if _, err := tx.Exec(ctx, sqlDeleteContents, packageID); err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to delete package contents: %v", err))
		return err
	}
	if _, err := tx.Exec(ctx, sqlDeletePackage, packageID); err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to delete package: %v", err))
		return err
	}

	if err := tx.Commit(ctx); err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to commit transaction: %v", err))
		return err
	}
	return nil
}
...
func (r *GoodsRepository) GetAllProducts(ctx context.Context) ([]models.Product, error) {
	...
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to get all products: %v", err))
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &p.Description, &p.Price, &p.ImageURL, &p.SKU); err != nil {
			r.log.Log("error", fmt.Sprintf("Failed to scan product: %v", err))
			continue
		}
		products = append(products, p)
	}
	if err = rows.Err(); err != nil {
		r.log.Log("error", fmt.Sprintf("Failed during rows iteration: %v", err))
		return nil, err
	}
	return products, nil
}
...
func (r *GoodsRepository) GetPackageByID(ctx context.Context, packageID int64) (*models.Package, []models.PackageContent, error) {
	...
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to get package by ID: %v", err))
		return nil, nil, err
	}
	...
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to get package contents: %v", err))
		return nil, nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.PackageID, &c.ProductID, &c.Quantity); err != nil {
			r.log.Log("error", fmt.Sprintf("Failed to scan package content: %v", err))
			continue
		}
...
func (r *GoodsRepository) ListPackages(ctx context.Context) ([]models.Package, error) {
	...
	rows, err := r.client.Query(ctx, sql)
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to list packages: %v", err))
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.PackageName, &p.Description); err != nil {
			r.log.Log("error", fmt.Sprintf("Failed to scan package: %v", err))
			continue
		}
...
func (r *GoodsRepository) GetProductsByPackageID(ctx context.Context, packageID int64) ([]models.PackageContent, error) {
	...
	if err != nil {
		r.log.Log("error", fmt.Sprintf("Failed to get products by package ID: %v", err))
		return nil, err
	}
	defer rows.Close()
